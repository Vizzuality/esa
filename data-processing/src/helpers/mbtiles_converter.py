...
class GeoTIFFConverter(MBTilesConverter):
    ...
    def _convert_to_mbtiles(self, input_path: Path, output_path: Path) -> None:
        ...
        try:
            console.print("🔧 Converting GeoTIFF to MBTiles...", style="white")
            subprocess.run(command, check=True)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"Error converting GeoTIFF with GDAL: {e}") from e

    def _add_overviews(self, mbtiles_path: Path, overview_levels: list[int]) -> None:
        command = [
            "gdaladdo",
            "-r",
            "average",
            mbtiles_path.as_posix(),
            *(str(o) for o in overview_levels),
        ]

        try:
            console.print("🗺️ Adding overviews to MBTiles...", style="white")
            subprocess.run(command, check=True)
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"Error adding overviews with GDAL: {e}") from e
...
class MBTilesConverterFactory:
    ...
    _converters: Dict[str, Type[MBTilesConverter]] = {
        ".tif": GeoTIFFConverter,
        ".tiff": GeoTIFFConverter,
        ".shp": VectorConverter,
